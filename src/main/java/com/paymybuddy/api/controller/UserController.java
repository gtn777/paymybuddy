...
public class UserController {
     ...
     * "getUserInfo:user instanceof  UsernamePasswordAuthenticationToken " + "<br><br>" +
     ...
     * SecurityContextHolder.getContext().getAuthentication().getDetails(); } else if
     * (user instanceof OAuth2AuthenticationToken) {
     ...
     * { StringBuffer loginInfo = new StringBuffer(); UsernamePasswordAuthenticationToken
     * token = ((UsernamePasswordAuthenticationToken) user); if (token.isAuthenticated())
     ...
     * 
     * @SuppressWarnings("unused") private OidcIdToken getIdToken(OAuth2User principal)
     * throws Throwable { if (principal instanceof DefaultOidcUser) { DefaultOidcUser
     * oidcUser = (DefaultOidcUser) principal; return oidcUser.getIdToken(); } return
     * null; } //
     * -----------------------------------------------------------------------------
     * ----------------------------
     * 
     ...
     * addNewBuddy(@RequestParam String username, @RequestParam String friend) throws
     * Throwable { FriendshipDto result = userService.addNewBuddy(username, friend);
